...
export async function createPersonneCentreInteret(a, lang = "fr") { await person(a.id_personne); await centre(a.id_centre_interet); await unique(a.id_personne, a.id_centre_interet); const r = await database.query("INSERT INTO personne_centre_interet (id_personne, id_centre_interet) VALUES ($1, $2) RETURNING id", [a.id_personne, a.id_centre_interet]); return getPersonneCentreInteretById(r.rows[0].id, lang); }
export async function updatePersonneCentreInteret(id, changes, lang = "fr") { const old = await raw(id); if (!old) return null; const a = new PersonneCentreInteret({ id, id_personne: old.id_personne, id_centre_interet: Object.hasOwn(changes, "id_centre_interet") ? changes.id_centre_interet : old.id_centre_interet }); await centre(a.id_centre_interet); await unique(old.id_personne, a.id_centre_interet, id); await database.query("UPDATE personne_centre_interet SET id_centre_interet = $1 WHERE id = $2", [a.id_centre_interet, id]); return getPersonneCentreInteretById(id, lang); }
export async function deletePersonneCentreInteret(id) { const a = await raw(id); if (!a) return null; await database.query("DELETE FROM personne_centre_interet WHERE id = $1", [id]); return a; }
